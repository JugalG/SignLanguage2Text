...
while True:
    ...
    if hands:
        ...
        if imgCrop is not None and not imgCrop.size == 0:
            ...
            # Assuming your classifier.getPrediction() returns only prediction and index
            ...
            cv2.putText(imgOutput, f"{labels[index]} {confidence_str}", (x, y - 26), cv2.FONT_HERSHEY_COMPLEX, 1.7, (255, 255, 255), 2)
    ...
    if key == ord("s"):
        ...
        print(f"{labels[index]} - Confidence: {confidence}")
    elif key == ord("k"):
        counter += 1
        sentence.append(" ")
